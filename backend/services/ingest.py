...
from ml.predictor import predict_threat, Protocol
from services.geo import GeoService, get_geo_service, reset_geo_service, GeoLocation, COUNTRY_CENTER_COORDS
from services.feeds import get_feed_service, ThreatIndicator, get_real_threat_ips, fetch_abuseipdb_threats, fetch_cloudflare_targets
...
SOURCE_IP_PREFIXES = [
    "1.", "27.", "36.", "49.", "61.",  # Asia
    "77.", "89.", "5.", "31.",          # Europe/Russia
    "138.", "179.", "152.",              # South America
    "41.", "102.", "196.",               # Africa
]

# Target IP prefixes (typically data centers, cloud providers)
TARGET_IP_PREFIXES = [
    "8.", "12.", "17.", "32.", "44.",   # US data centers
    "46.", "5.", "31.",                  # EU data centers
    "103.", "14.", "101.",               # APAC data centers
]
...
MALWARE_ATTACK_MAPPING = {
    "Dridex": {"attack_type": "HTTP Flood", "protocol": Protocol.HTTPS, "rate_range": (5000, 30000)},
    "Emotet": {"attack_type": "HTTP Flood", "protocol": Protocol.HTTP, "rate_range": (10000, 50000)},
    "QakBot": {"attack_type": "HTTPS Flood", "protocol": Protocol.HTTPS, "rate_range": (8000, 40000)},
    "Pikabot": {"attack_type": "SYN Flood", "protocol": Protocol.TCP, "rate_range": (20000, 80000)},
    "BumbleBee": {"attack_type": "UDP Flood", "protocol": Protocol.UDP, "rate_range": (30000, 100000)},
    "IcedID": {"attack_type": "DNS Amplification", "protocol": Protocol.DNS, "rate_range": (50000, 200000)},
    "TrickBot": {"attack_type": "SYN Flood", "protocol": Protocol.TCP, "rate_range": (15000, 60000)},
    "Unknown": {"attack_type": "UDP Flood", "protocol": Protocol.UDP, "rate_range": (10000, 50000)},
    # AbuseIPDB attack types
    "UDP Flood": {"attack_type": "UDP Flood", "protocol": Protocol.UDP, "rate_range": (30000, 100000)},
    "SSH Brute Force": {"attack_type": "SSH Brute Force", "protocol": Protocol.TCP, "rate_range": (1000, 5000)},
    "Port Scanning": {"attack_type": "Port Scanning", "protocol": Protocol.TCP, "rate_range": (500, 2000)},
    "Malicious Traffic": {"attack_type": "Malicious Traffic", "protocol": Protocol.TCP, "rate_range": (5000, 20000)},
}
...
def generate_threat_from_indicator(indicator: ThreatIndicator, target_weights: list[tuple[str, float]] = None) -> dict:
    """
    ...
    attack_config = MALWARE_ATTACK_MAPPING.get(
        malware, 
        MALWARE_ATTACK_MAPPING["Unknown"]
    )
    ...
    if should_use_real:
        ...
        if all_indicators:
            ...
            if len(threats) < MIN_THREATS_FOR_DEMO:
                ...
                print(f"  [HYBRID] Feed limit reached. Padding with {padding_needed} simulated events for demo.")
...
async def enrich_threat_with_geo(
    threat: dict,
...
def score_threat(threat: dict) -> float:
    ...
    probability = predict_threat(
        ...
        protocol_id=threat["protocol_id"]
    )
...
async def run_continuous_ingestion(
    ...
    """
    feed_type = "real feeds" if (use_real_feeds if use_real_feeds is not None else USE_REAL_FEEDS) else "mock data"
    print(f"Starting continuous ingestion (interval: {interval_seconds}s, batch: {batch_size}, source: {feed_type})")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of threats to ingest (default: 10)"
    )
    parser.add_argument(
        "--continuous",
        action="store_true",
    ...
    parser.add_argument(
        "--mock",
        action="store_true",
        help="Use mock data instead of real threat feeds"
    )
    parser.add_argument(
        "--real",
        action="store_true",
        help="Force use of real threat feeds"
    )
