...
async def get_attacks_since(
    db: AsyncSession = Depends(get_db),
    since_id: Optional[int] = Query(default=0, description="Get attacks after this ID"),
...
async def get_attack_statistics(
    ...
    for band, (low, high) in [("low", (0, 3)), ("medium", (3, 6)), ("high", (6, 8)), ("critical", (8, 10))]:
        band_query = select(func.count(AttackEvent.id)).where(
            AttackEvent.severity_score >= low,
            AttackEvent.severity_score < high if band != "critical" else AttackEvent.severity_score <= 10
        )
    ...
    avg_query = select(
        func.avg(AttackEvent.severity_score),
        func.avg(AttackEvent.packet_rate)
    )
