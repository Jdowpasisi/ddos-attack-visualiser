...
Use:
- Region: `sin` (or `bom` if available)
- PostgreSQL: Yes (free dev)
- Redis: No

This repository now includes `backend/fly.toml` with:
- `auto_stop_machines = false`
- `min_machines_running = 1`
- HTTP check on `/`
- 512 MB shared VM

Set secrets (never store keys in `fly.toml`):
...
```bash
curl https://ddos-map-backend.fly.dev/
curl https://ddos-map-backend.fly.dev/api/v1/attacks/stats
```

### 2) Frontend on Vercel

Production env defaults are included in `frontend/.env.production`:

```env
VITE_API_URL=https://ddos-map-backend.fly.dev
VITE_WS_URL=wss://ddos-map-backend.fly.dev
```
...
Recommended setup when prompted:
...
```bash
curl https://ddos-map-backend.fly.dev/
curl https://ddos-map-backend.fly.dev/ingestion/status

npm install -g wscat
wscat -c wss://ddos-map-backend.fly.dev/api/v1/ws/attacks
```

Open the dashboard URL from Vercel and confirm live arc updates.
